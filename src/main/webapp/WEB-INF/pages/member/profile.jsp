...
<%-- 
  Demonstrating JavaBean tags as required by Week 5 coursework topics.
  Normally EL (${userProfile.fullName}) is preferred, but the spec requires 
  jsp:useBean and jsp:getProperty in at least one page.
--%>
<jsp:useBean id="userProfile" class="com.gympulse.model.UserModel" scope="request" />
        ...
        <div class="bg-zinc-900/40 border border-zinc-800 rounded-2xl p-8 backdrop-blur-xl shadow-2xl relative overflow-hidden group">
            ...
            <form action="${pageContext.request.contextPath}/member/profile" method="POST" enctype="multipart/form-data" class="space-y-5 relative z-10">
                ...
                <div>
                    ...
                    <input type="text" name="fullName" class="w-full px-4 py-3 bg-zinc-950 border border-zinc-800 rounded-xl focus:border-zinc-600 outline-none text-white text-sm transition-colors shadow-inner" 
                           value='<jsp:getProperty name="userProfile" property="fullName" />' required>
                </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                    <div>
                        ...
                        <p class="w-full px-4 py-3 bg-zinc-950/50 border border-zinc-800/50 rounded-xl text-zinc-500 text-sm cursor-not-allowed select-none">${userProfile.email}</p>
                    </div>
                    <div>
